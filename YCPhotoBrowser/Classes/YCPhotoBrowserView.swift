...
open class YCPhotoBrowserView: UIView, UIScrollViewDelegate {
    
    /// 弱引用browser
    open weak var browser: YCPhotoBrowser?
    
    /// 询问当前数据总量
    open lazy var numberOfItems: () -> Int = { 0 }
    
    /// 返回可复用的Cell类。用户可根据index返回不同的类。本闭包将在每次复用Cell时实时调用。
    open lazy var cellClassAtIndex: (_ index: Int) -> YCPhotoCell.Type = { _ in
        YCPhotoImageCell.self
    }
    
    /// 刷新Cell数据。本闭包将在Cell完成位置布局后调用。
    open lazy var reloadCellAtIndex: (YCPhotoBrowser.ReloadCellContext) -> Void = { _ in }
    
    /// 自然滑动引起的页码改变时回调
    open lazy var didChangedPageIndex: (_ index: Int) -> Void = { _ in }
    
    /// Cell将显示
    open lazy var cellWillAppear: (YCPhotoCell, Int) -> Void = { _, _ in }
    
    /// Cell将不显示
    open lazy var cellWillDisappear: (YCPhotoCell, Int) -> Void = { _, _ in }
    
    /// Cell已显示
    open lazy var cellDidAppear: (YCPhotoCell, Int) -> Void = { _, _ in }
    
    /// 滑动方向
    open var scrollDirection: YCPhotoBrowser.ScrollDirection = .horizontal
    ...
    /// 显示中的Cell
    open var visibleCells = [Int: YCPhotoCell]()
    
    /// 缓存中的Cell
    open var reusableCells = [String: [YCPhotoCell]]()
    
    /// 入队
    private func enqueue(cell: YCPhotoCell) {
    ...
    /// 出队，没缓存则新建
    private func dequeue(cellType: YCPhotoCell.Type, browser: YCPhotoBrowser) -> YCPhotoCell {
        var cell: YCPhotoCell
        let name = String(describing: cellType.classForCoder())
        if var array = reusableCells[name], array.count > 0 {
            YCPhotoBrowserLog.middle("命中缓存！\(name)")
            cell = array.removeFirst()
            reusableCells[name] = array
        } else {
            YCPhotoBrowserLog.middle("新建Cell! \(name)")
    ...
    open func resetCells() {
        ...
        for index in (pageIndex - 1)...(pageIndex + 1) {
            ...
            YCPhotoBrowserLog.middle("Required class name: \(String(describing: clazz))")
            YCPhotoBrowserLog.middle("index:\(index) 出列!")
