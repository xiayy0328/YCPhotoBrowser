...
open class YCPhotoBrowser: UIViewController {
    
    /// 通过本回调，把图片浏览器嵌套在导航控制器里
    public typealias PresentEmbedClosure = (YCPhotoBrowser) -> UINavigationController
    ...
    /// 自实现转场动画
    open lazy var transitionAnimator: YCPhotoAnimatedTransitioning = YCPhotoFadeAnimator()
    
    /// 滑动方向
    open var scrollDirection: YCPhotoBrowser.ScrollDirection {
    ...
    open var cellClassAtIndex: (_ index: Int) -> YCPhotoCell.Type {
    ...
    public typealias ReloadCellContext = (cell: YCPhotoCell, index: Int, currentIndex: Int)
    ...
    /// Cell将显示
    open var cellWillAppear: (YCPhotoCell, Int) -> Void {
        set { browserView.cellWillAppear = newValue }
        get { browserView.cellWillAppear }
    }
    
    /// Cell将不显示
    open var cellWillDisappear: (YCPhotoCell, Int) -> Void {
        set { browserView.cellWillDisappear = newValue }
        get { browserView.cellWillDisappear }
    }
    
    /// Cell已显示
    open var cellDidAppear: (YCPhotoCell, Int) -> Void {
        set { browserView.cellDidAppear = newValue }
        get { browserView.cellDidAppear }
    }
    
    /// 主视图
    open lazy var browserView = YCPhotoBrowserView()
    
    /// 页码指示
    open var pageIndicator: YCPhotoBrowserPageIndicator?
    ...
    deinit {
        YCPhotoBrowserLog.high("deinit - \(self.classForCoder)")
    ...
    open func show(method: ShowMethod = .present(fromVC: nil, embed: nil)) {
        switch method {
        case .push(let inNC):
            let nav = inNC ?? YCPhotoBrowser.topMost?.navigationController
        ...
        case .present(let fromVC, let embed):
            ...
            let from = fromVC ?? YCPhotoBrowser.topMost
    ...
    open override func viewDidLoad() {
        ...
        browserView.browser = self
        transitionAnimator.browser = self
...
// MARK: - Status Bar/ Navigation Bar
extension YCPhotoBrowser {
...
// MARK: - 转场
extension YCPhotoBrowser: UIViewControllerTransitioningDelegate, UINavigationControllerDelegate {
