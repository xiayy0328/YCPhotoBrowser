...
class CustomViewController: BaseCollectionViewController {
    ...
    override func openPhotoBrowser(with collectionView: UICollectionView, indexPath: IndexPath) {
        ...
        browser.reloadCellAtIndex = { context in
            let photoCell = context.cell as? RawImageCell
            ...
            photoCell?.reloadData(placeholder: placeholder, urlString: urlString, rawURLString: rawURLString)
        }
        // 使用自定义的转场动画
        browser.transitionAnimator = CustomAnimatedTranstioning(previousView: { index -> UIView? in
...
/// 加上进度环的Cell
class RawImageCell: YCPhotoImageCell {
    /// 进度环
